...
GTCRN への切り替えは、実行中なら**再起動**、停止中なら**開始**で反映します。VST3版にはこれらの入力ノイズ
抑制は含まれません。
...
実行中なら**再起動**、停止中なら**開始**で反映します。
CLIで使う場合は `download-models.ps1 -Gtcrn` で `assets\gtcrn\` に取得できます。このフォルダや既存のモデルフォルダを `--gtcrn-model <dir>` で指定してください。
...
遅延を下げる**のが安全です。ピッチと入力・出力音量は通常画面で、Speaker IDは
声の選択欄の下にあるモデル設定内で、実行中も調整できます。
