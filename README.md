...
- **GUI 版（`vc-gui.exe`）** — 単体で使うためのデスクトップアプリです。
- **同梱 CLI（`vc-rs.exe`）** — 診断、自動化、リアルタイム変換、WAV ファイル
  変換に使えます。
...
1. ダウンロードした zip を展開します（**DLL は `vc-gui.exe` と `vc-rs.exe`
   と同じフォルダに置いたまま**にしてください）。
2. 通常は `vc-gui.exe` を起動します。CLI を使う場合は、そのフォルダで
   PowerShell を開きます。
