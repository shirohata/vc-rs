...
.DESCRIPTION
    ...
    Run AFTER building the CLI, e.g.:
        cargo build --release -p vc-cli --no-default-features --features windowsml,rnnoise,gtcrn
...
if (-not (Test-Path (Join-Path $DestDir 'vc-rs.exe'))) {
    throw "vc-rs.exe not found in $DestDir. Build it first: cargo build --release -p vc-cli --no-default-features --features windowsml,rnnoise,gtcrn"
}
