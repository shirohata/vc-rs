...
## Chunk msの変更に再読み込みが必要な理由

固定shapeは、モデルに渡す配列（テンソル）の寸法を決めて推論を最適化する仕組みです。
ネイティブTensorRTではチャンクと周辺コンテキストから各モデルの入力寸法を決め、
その寸法用のエンジンを繰り返し使います。[固定・可変shapeの比較](tensorrt_performance_ja.md)でも
固定shapeの性能上の利点が確認されています。固定するのは配列の寸法で、音声の内容ではありません。

例えばChunk msを100から200に変更すると、1回に進める音声の長さが変わります。
入力寸法が変わるモデルでは対応するTensorRTエンジンが必要になり、キャッシュに
適合するエンジンがあれば再利用し、なければ構築します。再読み込みのたびに必ず
エンジンを新規構築するわけではありません。OpenVINO GPU経路でもContentVecの入力を
固定していますが、RVCは動的shapeであり、固定化の範囲はバックエンド・モデルごとに異なります。

また、チャンク長は推論だけでなく、音声履歴、F0のフレーム対応、リサンプラのFIFO、
チャンクの接続処理、入出力バッファや遅延報告にも関わります。現在のvc-rsはこれらを
同じチャンク長に合わせて初期化するため、動的shapeのバックエンドでも、稼働中の
パイプラインに異なる長さをそのまま渡して切り替えることはできません。

変更後はGUIで変換を再起動し、VST3では **Load / Reload** を押してください。
CLIのリアルタイム変換は新しい設定で起動し直します。これはTensorRT自体が可変shapeを
扱えないという制約ではなく、固定shapeの最適化とストリーミング状態の整合を保つ
現在の実装上の選択です。内部の契約は[チャンクの処理周期](architecture.md#chunk-lifecycle)を参照してください。
